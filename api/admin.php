...
if ($action === 'users') {
    $result = $conn->query("SELECT id, username, email, role, credits, created_at FROM users");
...
if ($action === 'approve_template') {
    ...
    $conn->query("UPDATE templates SET approved = 1 WHERE id = $template_id");
    json_response(true, 'Template approved');
}
