...
if ($action === 'register') {
    ...
    $query = "INSERT INTO users (username, email, password, role, api_key, credits) VALUES ('$username', '$email', '$hashed', '$role', '$api_key', 100)";
    
    if ($conn->query($query)) {
        json_response(true, 'User created', ['api_key' => $api_key]);
    } else {
